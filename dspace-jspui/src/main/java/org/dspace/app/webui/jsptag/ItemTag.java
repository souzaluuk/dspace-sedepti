    ...
    private void render() throws IOException, SQLException, DCInputsReaderException
    {
        ...
        while (st.hasMoreTokens())
        {
            ...
            if (values.length > 0)
            {
                ...
                {
                    //Linhas para hipertextos de advisor1 e subject
                    if ("advisor1".equals(values[j].qualifier) && ("contributor".equals(values[j].element))){
                        browseIndex = "advisor";
                    }
                    if ("subject".equals(values[j].element) && null == values[j].qualifier){
                        browseIndex = "subject";
                    }
                    //Fim
                    ...
                    if (values[j] != null && values[j].value != null)
                    {
                        ...
                        String lattes = values[j].value;                            
                        if (lattes.length() >= 21){
                            String lattes1 = lattes.substring(0,21);
                            
                            if (lattes1.equals("http://lattes.cnpq.br")){
                                out.print("<a href=\"" + lattes + "\" target=\"_blank\">"
                                        + "<img src=\"/jspui/logo-lattes.png\" style=\"margin-left:7px;width:16px;height:18px;\">"
                                        + "</a>");
                            }
                        }
                    }
