      ...
      throws java.io.IOException
    {
      ...
      for (int i = 0; i < fieldCount; i++)
      {
         ...
         if (i < defaults.length)
         {
          ...
          if (!readonly)
          {
						// Delimitaou-se o tamanho do botão de exclusão
                       sb.append("<button style=\"width:45px;\" class=\"btn btn-danger col-md-2\" name=\"submit_")
                             .append(fieldName)
                             .append("_remove_")
                             .append(i)
                             .append("\" value=\"")
        	  ...
        	  sb.append("<span class=\"col-md-2\">TESTEZÃO1</span>");
          }
         }
         else
         {
         ...
         if(i != fieldCount)
                 ...
                 if (i < defaults.length)
                 {
                   ...
                   if (!readonly)
                   {
                 	  ...
                 	  sb.append("<span class=\"col-md-2\">TESTEZÃO3</span>");
                   }              
                 }
                 else
                 {
...
<%  //if not first page & step, show "Previous" button
    ...
    <%  } else { %>
    		<div class="col-md-4 pull-right btn-group">
				<%-- Para que ficassem alinhados, foi necessário trocar md-6 por md-6 --%>
                <input class="btn btn-default col-md-5" type="submit" name="<%=AbstractProcessingStep.CANCEL_BUTTON%>" value="<fmt:message key="jsp.submit.edit-metadata.cancelsave"/>"/>
				<input class="btn btn-primary col-md-5" type="submit" name="<%=AbstractProcessingStep.NEXT_BUTTON%>" value="<fmt:message key="jsp.submit.edit-metadata.next"/>"/>
    <%  }  %>
    		</div><br/>
</div>    		
    </form>
